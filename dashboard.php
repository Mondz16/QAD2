...
$sql = "SELECT s.id, s.level_applied, s.schedule_date, s.schedule_time, c.college_name, p.program_name 
        FROM schedule s 
        JOIN college c ON s.college_code = c.code
        JOIN program p ON s.program_id = p.id";
if ($year) {
    $sql .= " WHERE YEAR(s.schedule_date) = $year";
}
    ...
    <div class="wrapper">
        ...
        <aside id="sidebar">
            <div class="d-flex">
                ...
                <div class="sidebar-logo">
                    <a href="#"><img src="images/qad.png"style="width: 50px; height: 50px;" alt="USeP Logo">
                        QAD
                    </a>
                </div>
            </div>
        ...
        <div class="hair" style="height: 15px; background: linear-gradient(275.52deg, #973939 0.28%, #DC7171 100%);"></div>
            ...
            <div class="container text-center mt-4 ps-5">
                <p class="fw-bold" style="font-size: 1.5rem">SCHEDULE LIST</p>
                            ...
                            <?php
                                if ($result->num_rows > 0) {
                                    while($row = $result->fetch_assoc()) {
                                        echo "<div class='col-md-3 col-12 m-2 border rounded-2 text-start p-4'>
                                                <div ><label class='fw-bold'>College Name:</label> {$row['college_name']}</div>
                                                <div ><label class='fw-bold'>Program:</label> {$row['program_name']}</div>
                                                <div ><label class='fw-bold'>Level Applied:</label> {$row['level_applied']}</div>
                                                <div ><label class='fw-bold'>Schedule Date:</label> {$row['schedule_date']}</div>
                                                <div ><label class='fw-bold'>Schedule Time:</label> {$row['schedule_time']}</div>
                                            </div>";
                                    }
                                } else {
                                    echo "<p>No Schedule Implemented</p>";
                                }
                            ?>     
                        </div>
                    </div>

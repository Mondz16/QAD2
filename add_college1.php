                    ...
                    <div class="col-10 m-2 border rounded-2 bg-white">
                        <div class="mx-3 row">
                            <div class="col-12 text-start pt-4">
                                <label class="col-form-label" style="color: grey;">COLLEGE NAME</label>
                                ...
                                <label class="col-form-label" style="color: grey;">COLLEGE EMAIL</label>
                                <input type="text" class="form-control py-3" placeholder="ex. cic@usep.edu.ph">
                            </div>
                        </div>
                        <div class="mx-3 row">
                            <div class="col-12 text-start pt-3">
                            <label class="col-form-label" style="color: grey;">PROGRAM</label>
                            </div>
                            <div id=programList></div>
                        </div>
                        <div class="row mx-4 py-3">
                            <button class="btn btn-light border col-12 mb-2" data-bs-toggle="modal" data-bs-target="#modalOne">ADD PROGRAM</button>
                            <button class="col-12 cancel py-2 rounded-2 mb-2" data-bs-toggle="modal" data-bs-target="#modalTwo">REMOVE PROGRAM</button>
                            <button class="col-12 btn btn-success py-2">SUBMIT</button>
                        </div>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="modalOne" tabindex="-1" aria-labelledby="modalOneLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalOneLabel">ADD PROGRAM</h5>
                </div>
                <div class="modal-body">
                    <form>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="programLevel" placeholder="Enter program level">
                        </div>
                        <div class="mb-3">
                            <label for="dateReceived" class="form-label">DATE RECIEVED</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success" id="saveProgramBtn">ADD PROGRAM</button>
                </div>
            </div>
        </div>
    </div>
    <!-- Second Modal for listing and deleting programs -->
    <div class="modal fade" id="modalTwo" tabindex="-1" aria-labelledby="modalTwoLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTwoLabel">Manage Programs</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <ul class="list-group" id="programListModal">
                        <!-- Populated programs with delete buttons will appear here -->
                    </ul>
                </div>
                <div class="modal-footer">
    ...
    <script>
        ...
        // Function to update program list in both main content and modal
        function updateProgramList() {
            ...
            programListModal.innerHTML = '';

            // Populate lists
            programs.forEach((program, index) => {
                // Create and append program details to the main content
                const programDiv = document.createElement('div');
                programDiv.className = 'col-12 border rounded-2 text-start p-4 mt-2';
                programDiv.innerHTML = `
                    <p>
                        ${program.name} <br>
                        LVL: ${program.level} <br>
                        DATE RECEIVED: ${program.date}
                    </p>
                `;
                programList.appendChild(programDiv);

                // Create and append program name with delete button to the modal
                const listItemModal = document.createElement('li');
                listItemModal.className = 'list-group-item d-flex justify-content-between align-items-center';
                listItemModal.textContent = program.name;
                ...
                deleteButton.onclick = function() {
                    // Remove program from array and update lists
                    programs.splice(index, 1);
                    updateProgramList();
                };

                listItemModal.appendChild(deleteButton);
                programListModal.appendChild(listItemModal);
            });
        }

        // Save program details from the first modal
        document.getElementById('saveProgramBtn').addEventListener('click', function() {
            ...
            if (programName) {
                // Add program to the array
                programs.push({ name: programName, level: programLevel, date: dateReceived });

                // Update the program list in the main content and the modal
                updateProgramList();
                ...
                const modal = bootstrap.Modal.getInstance(document.getElementById('modalOne'));
                modal.hide();
            }
        });
    </script>
